...
const Certificate = () => {
  ...
  const Date = data && data.date.split("-").reverse().join("-");

  const contant = (
    <p>
      In recognition of {gender} <span>{data.username}</span> dedication and
      hard work, we are pleased to present this certificate as a symbol of your
      successful completion of the <span id="cc">{data.course}</span> Course.
      Your commitment to learning and achievement is commendable, and we wish
      you continued success in your journey.
    </p>
  );
